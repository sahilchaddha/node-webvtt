...
function parse (input, options) {
  ...
  const { meta = false, strict = false } = options;
...
function parseCue (cue, i, strict) {
  ...
  const times = lines[0]
    ...
    .map((el) => el.replace(',', '.'))
    .map((el) => {
      const splittedTime = el.split('.');

      if (splittedTime.length > 1) {
        if (splittedTime[1] === 'undefined') {
          splittedTime[1] = '00';
        }
        ...
        const splittedTimeUnit = splittedTime[0].split(':').map((timeUnit) => {
        ...
        splittedTime[0] = splittedTimeUnit.join(':');
      }
